...
{
    public class FilesAboutController : BaseController
    {
        #region Fields

        private readonly IFileAboutService _fileAboutService;

        #endregion Fields

        #region Ctor

        public FilesAboutController(IFileAboutService fileAboutService)
        {
            _fileAboutService = fileAboutService;
        }
        ...
        {
            var check = await _fileAboutService.GetByIdAsync(subidFile);
        ...
        {
            var result = await _fileAboutService.Delete(id);
            return Ok(result);
        }
        ...
        public async Task<IActionResult> CreateImage([FromForm] List<IFormFile> filesadd, Guid id, int width = 512, int height = 500)
        {
            ...
            {
                await _fileAboutService.InsertAsync(listEntity, id);
        ...
        public async Task<IActionResult> UpdateImage([FromForm] List<IFormFile> filesadd, Guid id, int width = 512, int height = 500)
        {
            ...
            if (listEntity.Count() > 0)
            {
                await _fileAboutService.UpdateAsync(listEntity, id);
                foreach (var item in listEntity)
                {
                    var tem = new Files();
                    tem.FileName = item.FileName;
        ...
        public async Task<IActionResult> DeleteImage(Guid id)
        {
            if (id != null)
            {
                var check = await _fileAboutService.GetByIdAsync(id);
