...
{
    public class FileProductController : BaseController
    {
        #region Fields
        ...
        public FileProductController(IFileProductService fileProductService)
        {
        ...
        {
            var check = await _fileProductService.GetByIdAsync(subidFile);
        ...
        {
            var result = await _fileProductService.Delete(id);
            return Ok(result);
        }

        [Route("create-image")]
        [HttpPost, DisableRequestSizeLimit]
        public async Task<IActionResult> CreateImage([FromForm] List<IFormFile> filesadd, Guid id, int width = 222, int height = 222)
        {
            ...
            {
                await _fileProductService.InsertAsync(listEntity, id);
        ...
        [Route("update-image")]
        [HttpPost, DisableRequestSizeLimit]
        public async Task<IActionResult> UpdateImage([FromForm] List<IFormFile> filesadd, Guid id, int width = 222, int height = 222)
        {
            if (filesadd == null || filesadd.Count == 0)
            ...
            if (listEntity.Count() > 0)
            {
                await _fileProductService.UpdateAsync(listEntity, id);
                foreach (var item in listEntity)
                {
                    var tem = new Files();
                    tem.FileName = item.FileName;
        ...
        public async Task<IActionResult> DeleteImage(Guid id)
        {
            if (id != null)
            {
                var check = await _fileProductService.GetByIdAsync(id);
        ...
        {
            var files = await _fileProductService.GetFilesAdmin(id);
            return Ok(files);
        }
        ...
        {
            var check = await _fileProductService.GetByNameAsync(id);
